...
def main(md_path, out_path):
    ...
    if not standards:
        # Tentar no diretório acima se rodando de dentro da pasta scripts
        standards = load_standard_items('../../Itens Reorganizado.md')
    if not standards:
        # Tentar no diretório do projeto se rodando da raiz
        standards = load_standard_items('Itens Reorganizado.md')
    ...
    for occ in occurrences:
        ...
        all_items.append({
            'code': code,
            'desc': std.get('desc', info.get('desc', occ['title'].split('(')[0].strip())),
            'qty_str': fmt(occ['total_value']),
            'unit': std.get('unit', info.get('unit', occ['total_unit']))
        })
    ...
    for code, d in consolidated.items():
        ...
        consol_list.append({
            'code': code,
            'desc': std.get('desc', info.get('desc', d['title'].split('(')[0].strip())),
            'qty_str': fmt(d['total_sum']),
            'unit': std.get('unit', info.get('unit', d['total_unit']))
        })
