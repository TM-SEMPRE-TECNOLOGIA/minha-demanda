...
def add_calc_table(doc, d, items_info, standards):
    htype     = d['htype']
    rows_data = d['rows']
    ...
    std = standards.get(d['code'], {})
    unit_str = std.get('unit', items_info.get(d['code'], {}).get('unit', tu))
    if std and std.get('desc'):
        title_txt = f"ITEM {d['code']} ({std['desc'].upper()})"
...
def add_itens_table(doc, code, total_val, items_info, standards):
    std = standards.get(code, {})
    desc_full = std.get('desc', items_info.get(code, {}).get('desc', ''))
    unit_str  = std.get('unit', items_info.get(code, {}).get('unit', ''))
...
def main(md_path, out_path):
    ...
    from parse_report import load_standard_items
    standards = load_standard_items('Itens Reorganizado.md')
    if not standards:
        standards = load_standard_items('../../Itens Reorganizado.md')
    ...
    for code, d in consolidated.items():
        add_calc_table(doc, d, items_info, standards)
        add_itens_table(doc, code, d['total_sum'], items_info, standards)
    ...
    print(f'[!] Execute: python3 fix_xml.py {out_path}')
